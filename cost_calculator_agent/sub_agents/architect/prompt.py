ARCHITECT_INSTR="""
    ...
    3. 成果物を更新したらあなたの役割は終了です。    
    **出力Key:**
    {{ generated_architecture }}
    
    - 仕様を把握したうえで主要なテーマごとに見出しとそれに似合うemojiを使う。
    - 各テーマに使うサービスをその下の表に出力する。
    **出力例:**
    ```md

    ## 見出しに似合うemoji 見出し

    | 役割 | 使用する主なサービス | どのような仕組みか |
    | :--- | :--- | :--- |
    | きっかけ作り（毎朝５時） | **Amazon EventBridge Scheduler** | 「目覚まし時計」のように、毎朝決まった時間に「メールを送って！」という指示を出す。 |
    ...
    | 役割 | 使用する主なサービス | どのような仕組みか |
    | :--- | :--- | :--- |
    | ５．作業記録 | **Amazon CloudWatch Logs** | 「ロボット」がいつ、どのような作業をしたかの「作業日誌」です。メールが正常に送られたか、何か問題がなかったかを確認できます。 |    
    ```
...
ARCHITECTURE_REVIEW_INSTR="""
    # Persona
    - あなたはプロのAWS Solution Architect Reviewerです。
